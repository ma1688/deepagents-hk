...
from hkex_agent.cli.config import COLORS, console
from hkex_agent.cli.file_ops import FileOpTracker, build_approval_preview
from hkex_agent.cli.input import parse_file_mentions
from hkex_agent.cli.ui import (
    TokenTracker,
    format_tool_display,
...
def is_safe_shell_command(action_request: dict) -> bool:
    """Check if a shell command is safe to auto-approve.
    
    Args:
        action_request: The action request dictionary containing tool information.
        
    Returns:
        True if the command is safe to auto-approve, False otherwise.
    """
    tool_name = action_request.get("name") or action_request.get("tool")
    if tool_name != "shell":
        return False
    
    # Extract command using the same method as prompt_for_tool_approval
    ...
    # Check if command starts with "date" (safe read-only command)
    # This covers all date command variations like:
    # - date
    # - date +%Y
    # - date +%m
    # - date +%Y%m%d
...
async def execute_task(
    ...
    stream_input = {"messages": [HumanMessage(content=final_input)]}
        ...
        while True:
            ...
            if interrupt_occurred and hitl_request:
                ...
                if session_state.auto_approve:
                    ...
                    decisions = []
                    for action_request in hitl_request.get("action_requests", []):
                        # Auto-approve safe shell commands (like date)
                        if is_safe_shell_command(action_request):
                            description = action_request.get("description", "tool action")
                            console.print()
                            console.print(f"  [dim]⚡ {description} (auto-approved)[/dim]")
                            decisions.append({"type": "approve"})
                        else:
