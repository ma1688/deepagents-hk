...
**最近更新** (2025-12-03):
- 🔧 **自定义模型支持**：设置面板新增自定义模型输入框，可使用任意模型名称（如 `Pro/deepseek-ai/DeepSeek-V3`）
- 🔌 **模型连接测试**：设置面板新增"测试连接"开关，验证模型配置是否正确可用
- 📊 **Max Tokens 自由输入**：Max Tokens 从滑块改为文本框，支持任意值输入（上限 100 万）
- 👤 **用户注册功能**：新增用户自助注册页面，支持用户名/邮箱/密码注册，bcrypt 加密存储
...
- 🎛️ **配置预设模板**：内置 4 种预设（默认/深度分析/快速摘要/创意报告）
- ✅ **自动审批开关**：新增工具调用自动审批设置，可在配置面板中切换
- 📎 **文件下载链接**：Agent 生成的文件（.md, .pdf 等）自动提供下载按钮
...
| 配置项 | 类型 | 说明 |
|--------|------|------|
| API Provider | 下拉选择 | SiliconFlow / OpenAI / Anthropic |
